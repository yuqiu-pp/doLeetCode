...
public class SolutionGraph {
    ...
    /**
     *  执行用时 : 18 ms, 在Find the Town Judge的Java提交中击败了37.26% 的用户
        内存消耗 : 74.4 MB, 在Find the Town Judge的Java提交中击败了32.47% 的用户
     */
    public int findJudge(int N, int[][] trust) {
        int[][] graphMatix = createMatixGraph(N, trust);
        int obj = -1;
        // 遍历图，找符合条件1和2的顶点，入栈
        for (int i = 0; i < N; i++) {
            int num = 0;
    ...
    /**
     *  执行用时 : 19 ms, 在Find the Town Judge的Java提交中击败了35.76% 的用户
        内存消耗 : 82.2 MB, 在Find the Town Judge的Java提交中击败了14.52% 的用户
     */
    ...
    // 创建图 邻接矩阵存储
    public int[][] createMatixGraph(int num, int[][] src){
    ...
    // 从0开始
    public int[][] createMatixGraph1(int num, int[][] src){
        int[][] graph = new int[num][num];
    ...
    public int[] findOrder1(int numCourses, int[][] prerequisites) {
        ...
        for (LinkedList<Integer> g : graph){
            ...
            for (LinkedList<Integer> h : graph){
                // 没有边的点入栈
                if (h.size() == 1 && !map.containsKey(h.get(0))){
                    stack.push(h.get(0));
                }
                // 没有遍历到的有边点，有路径到散列表中的值
                if (h.size() > 1 && !map.containsKey(h.get(0))){
                    // 遍历h
                    for (Integer hh : h){
                        // 顺序 ？？
    ...
    /**
     *  执行用时 : 18 ms, 在Course Schedule II的Java提交中击败了58.88% 的用户
     内存消耗 : 47.9 MB, 在Course Schedule II的Java提交中击败了65.13% 的用户
     */
    ...
    /**
     *  执行用时 : 342 ms, 在Course Schedule II的Java提交中击败了5.18% 的用户
        内存消耗 : 49.5 MB, 在Course Schedule II的Java提交中击败了54.36% 的用户
     */
    ...
    public int[] findOrder3(int numCourses, int[][] prerequisites){
        // 邻接矩阵
        int[][] graph = createMatixGraph1(numCourses, prerequisites);
        ...
        for (int m = 0; m < numCourses; m++)
        {
            for (int i = 0; i < numCourses; i++) {
                ...
                for (j = 0; j < numCourses; j++){
                    if (i != j){
                        int p = graph[j][i];
                        if (graph[j][i] != 0 || map.containsKey(i)){
                            break;
                        }
                    }
                }
    ...
    public static void main(String[] args) {
        ...
        // int[][] trust = {{1,3}, {2,3}};
        // int n = 3;
        // int m = solution.findJudge(n, trust);
        ...
        // int n = 700;
        int[] rst = solution.findOrder(n, trust);
        for (int r : rst){
