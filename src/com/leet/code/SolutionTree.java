...
public class SolutionTree {
    ...
    /**
     *  执行用时 : 8 ms, 在N-ary Tree Level Order Traversal的Java提交中击败了67.53% 的用户
        内存消耗 : 53.6 MB, 在N-ary Tree Level Order Traversal的Java提交中击败了74.22% 的用户
     */
    // 广度优先遍历  BFS
    // currList 记录当前层的节点
    ...
    /**
     *  执行用时 : 4 ms, 在N-ary Tree Level Order Traversal的Java提交中击败了94.70% 的用户
        内存消耗 : 57.6 MB, 在N-ary Tree Level Order Traversal的Java提交中击败了48.23% 的用户
     */
    // 递归
    ...
    /**
     *  执行用时 : 63 ms, 在Minimum Height Trees的Java提交中击败了59.77% 的用户
        内存消耗 : 51.8 MB, 在Minimum Height Trees的Java提交中击败了82.19% 的用户
     */
    ...
    // 题理解错误，返回的是最短路径上的根节点
    // 题目要求的是，整棵树有最小高度，然后返回根节点
    public List<Integer> findMinHeightTrees00(int n, int[][] edges) {
        List<Integer> rst = new ArrayList<>();
        if (n == 0){
            return rst;
        }
